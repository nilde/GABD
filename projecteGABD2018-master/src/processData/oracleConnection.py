...
class oracleConnexion(nc.noConnexion):
  ...
  def insertVectorDataset(self, nameDataset, fileName, params ):
    ...
    """
    #vector si que funciona
    tipus = "vector"
    query = 'insert into DATASETS(NOM, TOTAL, TYPE) values (:1,:2,:3)'
    ...
    for row in features:
      ...
      query = 'insert into VECTOR(ID,VECTORVALUES,ID_DATASET,CLASE) values (:1,:2,:3,:4)'
      self.cursor.execute(query, (id, feature,nameDataset, label))
    query = 'update DATASETS set TOTAL = '+str(id)+' where NOM = \''+nameDataset+'\''
